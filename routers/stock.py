...
def list_produits(request: Request, q: Optional[str] = None, limit: int = 50):
    ...
    with get_db() as conn:
        rows = conn.execute(
            """SELECT p.id, p.reference, p.designation, p.unite, p.description,
                      COALESCE(SUM(l.quantite_restante),0) as stock_total,
                      MIN(CASE WHEN l.quantite_restante>0 THEN l.date_entree END) as date_fifo,
...
def get_produit(produit_id: int, request: Request):
    ...
    with get_db() as conn:
        ...
        empls = conn.execute(
            """SELECT emplacement,
                      SUM(quantite_restante) as quantite,
                      MIN(CASE WHEN quantite_restante>0 THEN date_entree END) as date_fifo_empl,
                      s.derniere_inventaire,
                      s.commentaire
               FROM lots_stock l
        ...
        for e in empls:
            ...
            if e["derniere_inventaire"]:
                try:
                    delta = (now - datetime.fromisoformat(e["derniere_inventaire"])).days
...
async def create_produit(request: Request):
    ...
    with get_db() as conn:
        try:
            ...
            raise HTTPException(409, "Référence déjà existante")
...
def get_emplacement(emplacement: str, request: Request):
    ...
    with get_db() as conn:
        ...
        mvts = conn.execute(
            """SELECT m.*, p.reference, p.designation
               ...
               WHERE m.emplacement=? ORDER BY m.created_at DESC LIMIT 30""",
            (emplacement,),
        ).fetchall()
    ...
    for r in refs:
        ...
        if d["derniere_inventaire"]:
            try:
                d["alerte_inventaire"] = (
                    now - datetime.fromisoformat(d["derniere_inventaire"][:19])
