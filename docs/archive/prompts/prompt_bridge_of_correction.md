# Prompt Windsurf — Correction du bridge : push d'OFs depuis Access

## Contexte

Les 3 prompts précédents ont été exécutés. La table `api_keys`, les endpoints de gestion
des clés dans `settings.py`, et l'onglet UI dans `/settings` sont corrects — ne pas y toucher.

`app/routers/api_bridge.py` a été créé avec de mauvais endpoints (saisie de production).
L'objectif réel : permettre à un script Access de pousser des **Ordres de Fabrication (OFs)**
directement dans `planning_entries` (onglet "MyProd > OF"), avec liaison optionnelle à un
dossier planning déjà créé manuellement via `numero_of`.

Le router est importé dans `main.py` (ligne 67) mais PAS encore inclus via `include_router`.
**Ne jamais modifier DB_PATH ni déplacer data/production.db.**
...
Cherche le bloc `app.include_router(...)` existant et ajoute à la suite :
```python
...
Après modification, vérifie que `GET /api/bridge/health` retourne `{"status": "ok"}` sans clé API.
Vérifie que `POST /api/bridge/of` sans header `X-Api-Key` retourne bien un 401.

**Ne rien toucher à :** `production_data`, `app/routers/fabrication.py`, `app/routers/planning.py`, `data/production.db`, `DB_PATH`.
