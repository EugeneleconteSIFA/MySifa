...
def _machines_csv_to_list(s: Optional[str]) -> List[str]:
    if not s:
        return []
    # Support historique : ancienne valeur "Cohésio 1" -> ["Cohésio 1"].
    # Support nouveau format : "Cohésio 1 · DSI" -> ["Cohésio 1", "DSI"].
    parts = [p.strip() for p in str(s).split(_MACHINES_SEP)]
    return [p for p in parts if p]


def _machines_list_to_csv(machines: Optional[List[str]]) -> Optional[str]:
    if not machines:
        return None
    seen: List[str] = []
    for m in machines:
...
def _normalize_op_spec(item: Any) -> tuple:
    ...
    for r in rows:
        for m in _machines_csv_to_list(r["machines_csv"]):
            if m not in union:
                union.append(m)
    if not union:
        # Ne touche pas au champ (garde la valeur existante pour rétrocompat).
        return
...
def update_op(event_id: int, op_id: int, body: OpUpdateBody, request: Request):
    ...
    with get_db() as conn:
        ...
        for k, v in body.model_dump(exclude_unset=True).items():
            ...
            if k == "machines":
                # Réservé aux admins (l'opérateur ne réassigne pas la machine
                # d'une op sur laquelle il saisit).
                if maint_role != "admin":
