...
async function loadUnread(){
  try{
    const r=await api('/api/qualite/unread-total');
    if(r.ok){const d=await r.json();S.unread=d.unread||0;updateUnreadBadges();}
  }catch(e){}
}
function updateUnreadBadges(){
  const els=['sb-unread','sb-unread2','fab-badge'].map(id=>document.getElementById(id));
  els.forEach(el=>{
    if(!el)return;
