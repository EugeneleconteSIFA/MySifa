...
function _fabPrintWin(title, pageSize, css, bodyHtml){
  ...
  w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>'+title+'</title><style>'
    ...
    +bodyHtml+'<script>window.onload=function(){window.focus();window.print();}<\/script><script src="/static/mysifa_impersonate.js?v=2"><\/script></body></html>');
  w.document.close();
}
...
function renderMain(){
  ...
  const isAdminUserMain = S.user && ((S.user.role==='superadmin'||S.user.role==='administration'||S.user.role==='administration_ventes'||S.user.role==='administration_technique'||S.user.role==='direction'));
...
function renderRepiquageDossierView(){
  ...
  const _isAdminCp = S.user && ((S.user.role==='superadmin'||S.user.role==='administration'||S.user.role==='administration_ventes'||S.user.role==='administration_technique'||S.user.role==='direction'));
...
async function init(){
  ...
  const isAdm = (user.role==='superadmin'||user.role==='administration'||user.role==='administration_ventes'||user.role==='administration_technique'||user.role==='direction');
